#!/usr/bin/env python3
"""Single-pass three-way benchmark: Two-step vs Merged-v2 vs Merged-v3.

Reads Two-step results from lpd_results.csv (no re-run needed).
Runs Merged-v2 and Merged-v3 ONNX detection in one pass per image.
Shares PARSeq session across both models.

Output: lpd_three_way.csv
...
def main():
    ...
    ap.add_argument("--out",   default=str(SCRIPT_DIR/"lpd_three_way.csv"))
    ...
    print(f"[init] v2: {args.v2}", flush=True)
    print(f"[init] v3: {args.v3}", flush=True)
